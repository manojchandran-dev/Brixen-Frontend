...
/// Global company categories — server-generated id already looks like
/// "COCAT65618502479" (COCAT + 11 digits), so unlike expense categories
/// there's no separate display-code derivation needed; the id IS the code.
class CompanyCategoriesRemoteDatasource implements RemoteMasterDatasource {
  const CompanyCategoriesRemoteDatasource();

  @override
  Future<MasterItem> getById(String id) async {
    try {
      final resp = await _dio.get(ApiEndpoints.companyCategoryById(id));
  ...
  Future<MasterItem> create({
    ...
    String? fullForm,
    bool isActive = true,
    String? companyId, // unused: company categories are global
  }) async {
    try {
  ...
  Future<void> delete(String id) async {
    try {
      await _dio.delete(ApiEndpoints.companyCategoryById(id));
    } on DioException catch (e) {
      throw mapDioError(e);
    }
  }
