...
/// The companyAdmin/employee Report tab — same design as superAdmin's
/// [ReportsBody] (which this was duplicated from, not extracted from — the
/// original stays untouched, same precedent as `app_menu_body.dart` and
/// `master_type_body.dart`), sourced from the same `reportsSummaryProvider`.
/// `GET /reports/summary` is already scoped to the caller's own company by
/// `CompanyScopeInterceptor` for any non-superAdmin session, so no
/// company-specific query logic is needed here — only a separate,
/// independently-editable component per [ReportHomePage]'s role switch.
...
class CompanyReportsBody extends ConsumerStatefulWidget {
  const CompanyReportsBody({super.key});

  @override
  ConsumerState<CompanyReportsBody> createState() => _CompanyReportsBodyState();
}

class _CompanyReportsBodyState extends ConsumerState<CompanyReportsBody> {
          ...
          while (cursor.isBefore(limit) && months.length < 24) {
            ...
            months.add(
              _Bucket(
                ...
                DateFormat('MMM').format(cursor).substring(0, 1),
              ),
            );
            cursor = end;
          }
  ...
  Widget build(BuildContext context) {
    ...
    return summaryAsync.when(
      ...
      error: (e, _) => ErrorCard(
        error: e,
        onRetry: () => ref.invalidate(reportsSummaryProvider(query)),
      ),
  ...
  Widget _buildContent(
    ...
    final sortedCumulative = [...summary.cumulativeProfit]
      ..sort((a, b) => a.date.compareTo(b.date));
    final cumulativeProfit = List<double>.filled(buckets.length, 0);
    int cumPtr = 0;
    double lastCumValue = 0;
    for (int i = 0; i < buckets.length; i++) {
      while (cumPtr < sortedCumulative.length &&
          sortedCumulative[cumPtr].date.isBefore(buckets[i].endExclusive)) {
    ...
    final topCategories = summary.topExpenseCategories
        .map((c) => MapEntry(c.label, c.amount))
        .toList();
    final statusEntries = summary.salesByStatus
        .map((s) => MapEntry(s.status, s.count))
        .toList();
    final topMethods = summary.salesByPaymentMethod
        .map((m) => MapEntry(m.label, m.amount))
        .toList();
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Sales minus expenses for $_rangeLabel',
                        style: const TextStyle(
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _TrendChart(
                buckets: buckets,
                salesValues: salesByBucket,
                expenseValues: expensesByBucket,
              ),
            ],
          ),
        ),
            ...
            child: Column(
              ...
              children: [
                ...
                ...topCategories.asMap().entries.map((entry) {
                  ...
                  return Padding(
                    padding: EdgeInsets.only(
                      bottom: i == topCategories.length - 1 ? 0 : 14,
                    ),
                    child: _CategoryBar(
                      label: cat.key,
                      amount: cat.value,
                  ...
                  children: [
                    _DonutChart(
                      ...
                      colors: List.generate(
                        ...
                        (i) => _reportPalette[i % _reportPalette.length],
                      ),
                      total: salesCount,
                    ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: statusEntries.asMap().entries.map((entry) {
                          ...
                          final s = entry.value;
                          final color =
                              _reportPalette[i % _reportPalette.length];
                          return Padding(
                            padding: EdgeInsets.only(
                              bottom: i == statusEntries.length - 1 ? 0 : 10,
                            ),
                            child: Row(
                              children: [
                                Container(
            ...
            child: Column(
              ...
              children: [
                ...
                ...topMethods.asMap().entries.map((entry) {
                  ...
                  return Padding(
                    padding: EdgeInsets.only(
                      bottom: i == topMethods.length - 1 ? 0 : 14,
                    ),
                    child: _CategoryBar(
                      label: m.key,
                      amount: m.value,
            ...
            child: Column(
              ...
              children: [
                ...
                ...topCustomers.asMap().entries.map((entry) {
                  ...
                  return Padding(
                    padding: EdgeInsets.only(
                      bottom: i == topCustomers.length - 1 ? 0 : 14,
                    ),
                    child: _RankedRow(
                      rank: i + 1,
                      label: c.key,
...
class _TrendChartState extends State<_TrendChart> {
  ...
  String _rangeLabel(_Bucket b) {
    final lastDay = b.endExclusive.subtract(const Duration(days: 1));
    final fmt = DateFormat('d MMM');
    if (lastDay.difference(b.start).inDays <= 0) return fmt.format(b.start);
    return '${fmt.format(b.start)} – ${fmt.format(lastDay)}';
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        // Tap-to-reveal detail — the bucket's date range plus its exact
        // Sales/Expenses values, since the bars alone only show relative
        // height, not what they actually reached.
        AnimatedSize(
              ...
              : Container(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: Text(
                          _rangeLabel(widget.buckets[selected]),
                          style: TextStyle(
                            fontSize: 11.5,
                            fontWeight: FontWeight.w700,
                            color: AppColors.ink,
                      ...
                      Text(
                        '₹${fmt.format(widget.salesValues[selected])}',
                        style: TextStyle(
                          fontSize: 11.5,
                          fontWeight: FontWeight.w700,
                          color: AppColors.ink,
                      ...
                      Text(
                        '₹${fmt.format(widget.expenseValues[selected])}',
                        style: TextStyle(
                          fontSize: 11.5,
                          fontWeight: FontWeight.w700,
                          color: AppColors.ink,
        ...
        TweenAnimationBuilder<double>(
          key: ValueKey(
            '${widget.buckets.length}-${widget.salesValues.join()}-${widget.expenseValues.join()}',
          ),
                ...
                SizedBox(
                  ...
                  child: Row(
                    ...
                    children: List.generate(widget.buckets.length, (i) {
                      final sH =
                          ((widget.salesValues[i] / safeMax) *
                                  _chartHeight *
                                  progress)
                              .clamp(
                                widget.salesValues[i] > 0 ? 3.0 : 0.0,
                                _chartHeight,
                              );
                      final eH =
                          ((widget.expenseValues[i] / safeMax) *
                                  _chartHeight *
                                  progress)
                              .clamp(
                                widget.expenseValues[i] > 0 ? 3.0 : 0.0,
                                _chartHeight,
                              );
          ...
          child: Stack(
            ...
            children: [
              SizedBox(
                ...
                height: 110,
                child: CustomPaint(
                  painter: _GaugePainter(fraction: animatedFrac, color: color),
                ),
              ),
              Padding(
...
class _GaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final midOuter =
        center +
        Offset(cos(midAngle), sin(midAngle)) * (radius + strokeWidth / 2 + 3);
    final midInner =
        center +
        Offset(cos(midAngle), sin(midAngle)) * (radius - strokeWidth / 2 - 3);
