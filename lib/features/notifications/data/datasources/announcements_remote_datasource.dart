...
class AnnouncementsRemoteDatasource {
  ...
  AnnouncementModel _one(Response resp) =>
      AnnouncementModel.fromJson((resp.data['data'] ?? resp.data) as Map<String, dynamic>);
  ...
  Map<String, dynamic> _body(AnnouncementModel a) => {
        ...
        if (a.ctaTarget == OpenOnTapTarget.specificPage) 'specific_page_route': a.specificPageRoute,
        'status': a.status.name,
        if (a.scheduledAt != null) 'scheduled_at': a.scheduledAt!.toUtc().toIso8601String(),
      };

  Future<List<AnnouncementModel>> getAll() async {
    try {
      final resp = await _dio.get(ApiEndpoints.announcements, queryParameters: {'page': 1, 'limit': 100});
  ...
  Future<AnnouncementModel> update(AnnouncementModel a) async {
    try {
      return _one(await _dio.put(ApiEndpoints.announcementById(a.id), data: _body(a)));
    } on DioException catch (e) {
      throw mapDioError(e);
    }
  }
